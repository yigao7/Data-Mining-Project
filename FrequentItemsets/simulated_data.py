# Build two kinds of market-basket models on simulated datasets
# Case 1:
...
# Create a basket for each user containing the business ids reviewed by this user. If a business was reviewed more than once by a reviewer, consider this product was rated only once. 
# Case 2:
...
# Create a basket for each business containing the user ids that commented on this business. 
...
if __name__ == '__main__':
    ...
    user_basket = record.groupByKey()\
                        .mapValues(lambda x: (list(set(x))))
    bus_basket = record.map(lambda x: (x[1], x[0]))\
                       .groupByKey() \
                       .mapValues(lambda x: (list(set(x))))
